...
class ValueAgent(Agent):
    ...
    @torch.no_grad()
    def __call__(self, state: torch.Tensor,action_space:List, device: str) -> List[int]:
        """
        ...
        if np.random.random() < self.epsilon:
            action = self.get_random_action(state,action_space)
        else:
            action = self.get_action(state, device)

        return action

    def get_random_action(self, state: torch.Tensor,action_space:List) -> int:
