...
pub fn find_fvg(fvgs: &VecDeque<FVG>, ts_event_left: u64, ts_event_right: u64, reversed: bool) -> Option<usize>
{
    let end_index = fvgs.len() - 1;
    for i in (0..fvgs.len()).rev()
    {
        if reversed && fvgs[i].ts_event > ts_event_left && fvgs[i].ts_event < ts_event_right
        {
            return Some(i);
        }
        else if !reversed && fvgs[end_index - i].ts_event > ts_event_left && fvgs[end_index - i].ts_event < ts_event_right
        {
            return Some(end_index - i);
        }
    }
    None
}
...
impl OrderBlockDetector {
    ...
    pub fn turtle_bull(&self) -> usize 
    {
        if self.blocks.len() < 6
        {
            return 0;
        }
        let cur_block = &self.blocks[self.blocks.len()-1];
        let back_1_block = &self.blocks[self.blocks.len()-2];
        let back_2_block = &self.blocks[self.blocks.len()-3];
        let back_3_block = &self.blocks[self.blocks.len()-4];
        let back_4_block = &self.blocks[self.blocks.len()-5];
        let bull_bear_condition = !cur_block.is_bull && back_1_block.is_bull && !back_2_block.is_bull && back_3_block.is_bull && !back_4_block.is_bull;
        if !bull_bear_condition
        {
            return 0;
        }
        let high_low_condition = cur_block.value > back_2_block.value && back_2_block.value > back_3_block.value && back_3_block.value > back_1_block.value && back_4_block.value > cur_block.value;
        if !high_low_condition
        {
            return 0;
        }

        let touch_condition = self.low_array[self.low_array.len()-2] < back_1_block.value && self.close_array[self.close_array.len()-1] > back_1_block.value;
        let cur_candle_change = self.close_array[self.close_array.len()-1] - self.close_array[self.close_array.len()-2];
        let last_candle_change = self.close_array[self.close_array.len()-3] - self.close_array[self.close_array.len()-2];
        if touch_condition && cur_candle_change > last_candle_change 
        {
            return 1;
        }
        else
    ...
    pub fn turtle_bear(&self) -> usize 
    {
        ...
        let touch_condition = self.high_array[self.high_array.len()-2] > back_1_block.value && self.close_array[self.close_array.len()-1] < back_1_block.value;
        let cur_candle_change = self.close_array[self.close_array.len()-2] - self.close_array[self.close_array.len()-1];
        let last_candle_change = self.close_array[self.close_array.len()-2] - self.close_array[self.close_array.len()-3];
        if touch_condition && cur_candle_change > last_candle_change 
        {
            return 1;
        }
